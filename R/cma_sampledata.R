...
#    dat$epsY <- rnorm(N, sd=sig.trueY)
    ...
    hfunM <- function(z, ind1 = 1, ind2 = 2, ind3 = 4) 1/4*z[ind1] + 1/4*z[ind3] + 1/4*z[ind1]*z[ind2]
    hfunY <- function(z, ind1 = 1, ind2 = 2, ind3 = 4, ind4 = 5, ind5 = 6) rbinom(n = 1, size = 1,
                        p = plogis(1/3*((z[ind1]+3) + (z[ind2]+3) + 1/3*z[ind3] + 1/2*(z[ind1]+3)*(z[ind4]+3) +  1*(z[ind2]+3)*(z[ind5]+3)), 2, 0.2))
